jQuery(window).on("elementor/frontend/init", function () {
    ...
    elementorFrontend.hooks.addAction(
        "frontend/element_ready/wtz-addons-maps.default",
        function ($scope, $) {

            var mapElement = $scope.find(".wtz_addons_map_height");

            var mapSettings = mapElement.data("settings");

            var mapStyle = mapElement.data("style");

            var wtz_addonsMapMarkers = [];

            wtz_addonsMap = newMap(mapElement, mapSettings, mapStyle);
            ...
            function newMap(map, settings, mapStyle) {
                ...
                var markers = map.find(".wtz-addons-pin");
            ...
            function add_marker(pin, map, autoOpen, hoverOpen, hoverClose) {
                ...
                wtz_addonsMapMarkers.push(marker);

                //Used with Carousel Custom Navigation option
                if (customID) {
                    google.maps.event.addListener(marker, "click", function () {
                        console.log(customID);
                        var $carouselWidget = $(".wtz-addons-carousel-wrapper");
                            ...
                            $carouselWidget.map(function (index, item) {
                                ...
                                if (carouselSettings.navigation) {
                                    if (-1 != carouselSettings.navigation.indexOf("#" + customID)) {
                                        ...
                                        $(item).find(".wtz-addons-carousel-inner").slick("slickGoTo", slideIndex);
                                    }
                                }
                            })
                ...
                if (
                    pin.find(".wtz-addons-maps-info-title").html() ||
                    pin.find(".wtz-addons-maps-info-desc").html()
                ) {
                    // create info window
                            ...
                            if ($carouselWidget.length) {
                                $carouselWidget.map(function (index, item) {
                                    ...
                                    if (carouselSettings.navigation) {
                                        if (-1 != carouselSettings.navigation.indexOf("#" + customID)) {
                                            var slideIndex = carouselSettings.navigation.indexOf("#" + customID);
                                            $carouselWidget.find(".wtz-addons-carousel-inner").slick("slickGoTo", slideIndex);
                                        }
                                    }
                                })
            ...
            if (markerCluster) {
                var markerCluster = new MarkerClusterer(wtz_addonsMap, wtz_addonsMapMarkers, {
                    imagePath:
