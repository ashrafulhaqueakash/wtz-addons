...
namespace Wtz_Addons;

/**
 * Class Breadcrumb_Trail
 * @package Happy_Addons_Pro
 */
class Wtz_Addons_Breadcrumb_Trail {
	...
	public function __construct ( $args = array() ) {
		...
		$defaults = array(
			'list_tag' => 'ul',
			'list_class' => 'wtz-addons-breadcrumbs-items',
			'item_tag' => 'li',
			'item_class' => 'wtz-addons-breadcrumbs-item',
			'separator' => ' / ',
			'separator_class' => 'wtz-addons-breadcrumbs-separator',
			'home_icon' => '',
			'home_icon_class' => 'wtz-addons-breadcrumbs-home-icon',
			'show_on_front' => true,
			'network' => false,
			'show_title' => true,
			'labels' => array(),
	...
	public function trail () {
		...
		if ( 0 < $item_count ) {
			...
			foreach ( $this->items as $item ) {
				...
				$item = ! empty( $matches ) ? sprintf( '%s<span class="wtz-addons-breadcrumbs-text">%s</span>%s', $matches[1], $matches[2], $matches[3] ) : sprintf( '<span class="wtz-addons-breadcrumbs-text">%s</span>', $item );

				// Add list item classes.
				$item_class = ! empty( $this->args['item_class'] ) ? $this->args['item_class'] : '';

				if ( 1 === $item_position  )
					$item_class .= ' wtz-addons-breadcrumbs-start';

				elseif ( $item_count === $item_position )
					$item_class .= ' wtz-addons-breadcrumbs-end';
	...
	protected function set_labels () {
		...
		$defaults = array(
			'home' => esc_html__( 'Home', 'wtz-addons' ),
			'page_title' => esc_html__( 'Pages', 'wtz-addons' ),
			'error_404' => esc_html__( '404 Not Found', 'wtz-addons' ),
			'archives' => esc_html__( 'Archives', 'wtz-addons' ),
			// Translators: %s is the search query.
			'search' => esc_html__( 'Search results for: %s', 'wtz-addons' ),
			// Translators: %s is the page number.
			'paged' => esc_html__( 'Page %s', 'wtz-addons' ),
			// Translators: %s is the page number.
			'paged_comments' => esc_html__( 'Comment Page %s', 'wtz-addons' ),
			// Translators: Minute archive title. %s is the minute time format.
			'archive_minute' => esc_html__( 'Minute %s', 'wtz-addons' ),
			// Translators: Weekly archive title. %s is the week date format.
			'archive_week' => esc_html__( 'Week %s', 'wtz-addons' ),
	...
	protected function add_minute_hour_archive_items () {
		...
		if ( true === $this->args['show_title'] )
			$this->items[] = sprintf( $this->labels['archive_minute_hour'], get_the_time( esc_html_x( 'g:i a', 'minute and hour archives time format', 'wtz-addons' ) ) );
	}
	...
	protected function add_minute_archive_items () {
		...
		// Add the minute item.
		if ( true === $this->args['show_title'] )
			$this->items[] = sprintf( $this->labels['archive_minute'], get_the_time( esc_html_x( 'i', 'minute archives time format', 'wtz-addons' ) ) );
	}
	...
	protected function add_hour_archive_items () {
		...
		// Add the hour item.
		if ( true === $this->args['show_title'] )
			$this->items[] = sprintf( $this->labels['archive_hour'], get_the_time( esc_html_x( 'g a', 'hour archives time format', 'wtz-addons' ) ) );
	}
	...
	protected function add_day_archive_items () {
		...
		$year = sprintf( $this->labels['archive_year'], get_the_time( esc_html_x( 'Y', 'yearly archives date format', 'wtz-addons' ) ) );
		$month = sprintf( $this->labels['archive_month'], get_the_time( esc_html_x( 'F', 'monthly archives date format', 'wtz-addons' ) ) );
		$day = sprintf( $this->labels['archive_day'], get_the_time( esc_html_x( 'j', 'daily archives date format', 'wtz-addons' ) ) );
	...
	protected function add_week_archive_items () {
		...
		$year = sprintf( $this->labels['archive_year'], get_the_time( esc_html_x( 'Y', 'yearly archives date format', 'wtz-addons' ) ) );
		$week = sprintf( $this->labels['archive_week'], get_the_time( esc_html_x( 'W', 'weekly archives date format', 'wtz-addons' ) ) );
	...
	protected function map_rewrite_tags ( $post_id, $path ) {
		...
		if ( is_array( $matches ) ) {
			...
			foreach ( $matches as $match ) {
				...
				if ( '%year%' == $tag )
					$this->items[] = sprintf( '<a href="%s">%s</a>', esc_url( get_year_link( get_the_time( 'Y', $post_id ) ) ), sprintf( $this->labels['archive_year'], get_the_time( esc_html_x( 'Y', 'yearly archives date format', 'wtz-addons' ) ) ) );

				// If using the %monthnum% tag, add a link to the monthly archive.
				elseif ( '%monthnum%' == $tag )
					$this->items[] = sprintf( '<a href="%s">%s</a>', esc_url( get_month_link( get_the_time( 'Y', $post_id ), get_the_time( 'm', $post_id ) ) ), sprintf( $this->labels['archive_month'], get_the_time( esc_html_x( 'F', 'monthly archives date format', 'wtz-addons' ) ) ) );

				// If using the %day% tag, add a link to the daily archive.
				elseif ( '%day%' == $tag )
					$this->items[] = sprintf( '<a href="%s">%s</a>', esc_url( get_day_link( get_the_time( 'Y', $post_id ), get_the_time( 'm', $post_id ), get_the_time( 'd', $post_id ) ) ), sprintf( $this->labels['archive_day'], get_the_time( esc_html_x( 'j', 'daily archives date format', 'wtz-addons' ) ) ) );
